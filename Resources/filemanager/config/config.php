...
{
	$webMediaPath = THELIA_WEB_DIR.'media/documentssharing';
...
if (empty($base_url)) {
	...
	$base_url = $request->getSchemeAndHttpHost().preg_replace('!/documentssharing/filemanager/dialog.php.*$!', '', $_SERVER['REQUEST_URI']);
}
//Check for backward compatibility
if ($env !== "prod") {
	// path from base_url to base of upload folder for current env (with start and final /)
	$upload_dir = '/media/documentssharing/'.$env.'/upload/';

	// path from base_url to base of upload folder for current env (with start and final /)
	$thumbs_dir = '/media/documentssharing/'.$env.'/thumbs/';

	// path to file manager folder to upload folder for current env (with final /)
	$current_path = THELIA_WEB_DIR.'media'.DS.'documentssharing'.DS.$env.DS.'upload'.DS;
	...
	$thumbs_base_path = THELIA_WEB_DIR.'media'.DS.'documentssharing'.DS.$env.DS.'thumbs'.DS;
} else {
	// path from base_url to base of upload folder (with start and final /)
	$upload_dir = '/media/documentssharing/upload/';

	// path from base_url to base of upload folder (with start and final /)
	$thumbs_dir = '/media/documentssharing/thumbs/';

	// path to file manager folder to upload folder (with final /)
	$current_path = THELIA_WEB_DIR.'media'.DS.'documentssharing'.DS.'upload'.DS;

	// path to file manager folder to thumbs folder (with final /)
	// WARNING: thumbs folder should not be inside the upload folder
	$thumbs_base_path = THELIA_WEB_DIR.'media'.DS.'documentssharing'.DS.'thumbs'.DS;
}

// path from base_url to filemanager folder (with start and final /)
$filemanager_dir = '/documentssharing/filemanager/';
...
$config = array(
	...
	*/
	...
	'ext_file'                                => array( 'doc', 'docx', 'rtf', 'pdf', 'xls', 'xlsx', 'txt', 'csv', 'html', 'xhtml', 'psd', 'sql', 'log', 'fla', 'xml', 'ade', 'adp', 'mdb', 'accdb', 'ppt', 'pptx', 'odt', 'ots', 'ott', 'odb', 'odg', 'otp', 'otg', 'odf', 'ods', 'odp', 'css', 'ai', 'cdr', 'indd' ), //Files
